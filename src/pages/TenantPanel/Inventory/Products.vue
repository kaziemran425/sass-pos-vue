<template>
  <q-page class="q-pa-md bg-grey-2">
    <div class="row items-center q-mb-md">
      <div class="text-h5 text-weight-bold">Product Inventory</div>
      <q-space />
      ...
      <q-card style="min-width: 500px">
        ...
        <q-card-section class="q-gutter-y-md">
          ...
          <div class="row q-col-gutter-sm">
            <div class="col-6"><q-input outlined v-model="form.sku" label="SKU / Barcode" /></div>
            <div class="col-6"><q-select outlined v-model="form.category" :options="['Electronics', 'Groceries', 'Clothing']" label="Category" /></div>
          </div>
          <div class="row q-col-gutter-sm">
            <div class="col-6"><q-input outlined v-model.number="form.price" label="Selling Price" prefix="$" type="number" /></div>
            <div class="col-6"><q-input outlined v-model.number="form.cost" label="Cost Price" prefix="$" type="number" /></div>
          </div>
          <q-input outlined v-model.number="form.stock" label="Current Stock" type="number" />
        </q-card-section>
...
const columns = [
  ...
  { name: 'price', label: 'Price', field: 'price', format: val => `$${val}`, sortable: true },
  { name: 'stock', label: 'Stock', field: 'stock', align: 'center', sortable: true },
  { name: 'actions', label: 'Actions', field: 'actions', align: 'right' },
]

// Form State
const defaultForm = { id: null, name: '', sku: '', category: '', price: 0, cost: 0, stock: 0, image: '' }
...
onMounted(() => {
  ...
  if (stored) {
    ...
    // Static Mock Data
    products.value = [
      { id: 1, name: 'Wireless Mouse', sku: 'WM-001', category: 'Electronics', price: 25, cost: 15, stock: 45, image: '' },
      { id: 2, name: 'Cotton T-Shirt', sku: 'TS-BLK-M', category: 'Clothing', price: 12, cost: 5, stock: 8, image: '' },
    ]
  }
})
